...
import { ScriptBlock, TopicSuggestion } from '../types';
...
const ScriptDisplay: React.FC<ScriptDisplayProps> = ({ 
  ...
  const getDocStyles = () => `
    <style>
        /* Define Landscape Page settings for Word */
        @page Section1 {
  ...
  const handleExportDossierOnly = () => {
    const header = `
      ...
      <!-- COVER PAGE -->
      <h1>INTELLIGENCE DOSSIER: ${topic}</h1>
      <p><strong>MEDIAWAR.CORE V3.3 // RESEARCH DATA ONLY</strong></p>
      <p>Generated: ${new Date().toLocaleString()}</p>
      <hr/>

      <!-- AGENT A: RADAR -->
      <h2>AGENT A: RADAR INTERCEPT</h2>
      <div class="raw-content">${radarContent || 'No Radar Data Available'}</div>
      
      <div class="page-break"></div>

      <!-- AGENT B: ANALYST -->
      <h2>AGENT B: INTELLIGENCE DOSSIER</h2>
      <div class="raw-content">${analystContent || 'No Analyst Data Available'}</div>

      <div class="page-break"></div>

      <!-- AGENT C: ARCHITECT -->
      <h2>AGENT C: STRUCTURE BLUEPRINT</h2>
      <div class="raw-content">${architectContent || 'No Architect Data Available'}</div>
      
      </div></body></html>
    `;
    
    downloadDoc(`DOSSIER_${topic.replace(/[^a-z0-9]/gi, '_').substring(0, 30)}.doc`, header);
  };
  ...
  const handleExportScriptOnly = () => {
    const header = `
      ...
      <!-- COVER PAGE -->
      <h1>SCRIPT: ${topic}</h1>
      <p><strong>MEDIAWAR.CORE V3.3 // PRODUCTION SCRIPT</strong></p>
      <p>Generated: ${new Date().toLocaleString()}</p>
      <hr/>

      <!-- AGENT D: WRITER (THE SCRIPT) -->
      <h2>AGENT D: FINAL SCRIPT</h2>
    `;
    
    const scriptBody = script.map(block => `
      <div class="block">
        <div class="time">${block.timecode} [${block.blockType}]</div>
        <div class="visual">VISUAL: ${block.visualCue}</div>
        
        ${block.imageUrl ? `
          <div class="img-container">
            <img class="storyboard" src="${block.imageUrl}" alt="Storyboard Frame" style="width: 600px; height: auto;" />
          </div>
        ` : ''}

        <div class="audio">AUDIO (EN): ${block.audioScript}</div>
        <div class="russian">AUDIO (RU): ${block.russianScript}</div>
      </div>
    `).join('');
    ...
    downloadDoc(`SCRIPT_${topic.replace(/[^a-z0-9]/gi, '_').substring(0, 30)}.doc`, sourceHTML);
  };
  ...
  const handleExportExcel = () => {
    ...
    link.setAttribute("download", `TASK_${topic.replace(/[^a-z0-9]/gi, '_').substring(0, 30)}.csv`);
      ...
      <div className="bg-mw-gray/20 rounded-lg overflow-hidden border border-mw-slate/30">
         <div className="bg-mw-red/10 border-b border-mw-red/30 p-4 flex flex-col sm:flex-row items-center justify-between gap-4">
          <div>
            ...
            <p className="text-xs text-mw-slate mt-1 font-mono">MEDIAWAR.CORE V3.3 // EXPORT_READY</p>
          </div>
