        ...
        if (trimmed.startsWith('- ') || trimmed.startsWith('* ')) {
          ...
          return (
            <div key={idx} className="flex gap-2 pl-2">
              <span className="text-mw-red font-bold">›</span>
        ...
        if (/^\d+\./.test(trimmed)) {
           return (
             <div key={idx} className="flex gap-2 pl-2 mt-1">
               <span className="text-mw-slate font-bold">{trimmed.split('.')[0]}.</span>
        ...
        if (trimmed.includes(':') && trimmed.length < 100 && !trimmed.endsWith(':')) {
             ...
             return (
                 <div key={idx} className="text-gray-400">
                     <span className="text-mw-slate uppercase text-xs font-bold mr-2">{label}:</span>
                     {parseBold(val)}
                 </div>
             )
        }
